...
var app = {
  ...
  // On Cordova Android, cordova.file.applicationDirectory + 'www/' points to the app bundle.
  resolveChapterUrl: function (url) {
    try {
      if (window.cordova && cordova.file && cordova.file.applicationDirectory) {
        ...
        // Ensure we reference the packaged www folder
...
/* Testable exports (if running under Node/Jest) */
try {
  if (typeof module !== 'undefined' && module.exports) {
    // Minimal helpers to test theme logic and URL resolver
    var __helpers = {
      THEME_KEY: 'scicalc_theme',
      applyTheme: function (theme) {
        var html = typeof document !== 'undefined' ? document.documentElement : {};
        if (theme === 'light') {
          if (html && html.classList) html.classList.add('theme-light');
        } else {
          if (html && html.classList) html.classList.remove('theme-light');
        }
      },
      getStoredTheme: function () {
        try {
          return localStorage.getItem('scicalc_theme');
        } catch (e) {
          return null;
        }
      },
      resolveChapterUrl: app.resolveChapterUrl,
    };
    module.exports = __helpers;
    try { globalThis.__scicalc_test_helpers__ = __helpers; } catch (e) { /* ignore */ }
  }
} catch (e) {
  /* ignore in browser */
}
