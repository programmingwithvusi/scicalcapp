...
var physics = {
  ...
  heatEnergy: function (mass, cp, dt) {
    ...
    return {
      joules: j,
      calories: j / J_PER_CAL,
      kj: j / 1000,
      direction:
        j > 0 ? '🔺 Heating (energy absorbed)' : '🔻 Cooling (energy released)',
    };
  },
};
...
var fmt = {
  energy: function (value) {
    ...
    if (abs < 0.001 || abs >= 1e6) {
      return (
        sign +
        abs.toExponential(4).replace('e+', ' × 10^').replace('e-', ' × 10^−')
      );
    }
...
var app = {
  ...
  screenMeta: {
    ...
    chapter: { icon: '📚', title: 'Chapter', back: true },
  },
    ...
    if (navigator.getBattery) {
      navigator
        ...
        .then(function (battery) {
          app._renderBattery(Math.round(battery.level * 100), battery.charging);
        })
        .catch(function () {
  ...
  _renderBattery: function (level, charging) {
    ...
    if (bar) {
      ...
      bar.classList.add(
        level >= 50 ? 'bar-good' : level >= 20 ? 'bar-warn' : 'bar-crit',
      );
    }
  ...
  refreshNetwork: function () {
    ...
    var connType =
      typeof Connection !== 'undefined' && navigator.connection
    ...
    if (navEl) navEl.textContent = navigator.onLine ? 'true' : 'false';
  ...
  /* ═══════════════════════════════════════════════════════════════════
     In-app chapter viewer — fetch external chapter HTML and inject
     ═══════════════════════════════════════════════════════════════════ */
  openChapter: function (url, title) {
    // remember last requested chapter for retry
    app._lastChapter = { url: url, title: title };
    var content = document.getElementById('chapter-content');
    var titleEl = document.getElementById('chapter-title');
    var retryBtn = document.getElementById('chapter-retry');
    if (titleEl) titleEl.textContent = title || 'Chapter';
    if (content) content.innerHTML = '<div class="empty-note">Loading…</div>';
    app.showScreen('chapter');

    // Try to fetch the external HTML and inject the body content
    fetch(url, { cache: 'no-store' })
      ...
      .then(function (html) {
        ...
        var inject = bodyMatch ? bodyMatch[1] : html;
        if (content) {
          content.innerHTML = inject;
          // ensure iframe is hidden when we injected content
          var wrap = document.getElementById('chapter-iframe-wrap');
          var loading = document.getElementById('chapter-iframe-loading');
          if (wrap) wrap.classList.add('hidden');
          if (loading) loading.classList.add('hidden');
        }
        app.log('Loaded chapter: ' + url);
      })
      .catch(function (err) {
        app.log('Chapter load failed: ' + err.message, 'error');
        if (content) {
          content.innerHTML =
            '<div class="error-msg">Failed to load chapter via fetch. Attempting to open in iframe below.</div>';
          if (retryBtn) retryBtn.classList.remove('hidden');
          // fallback: show iframe with the URL so chapter's scripts/styles can run
          var iframe = document.getElementById('chapter-iframe');
          var wrap = document.getElementById('chapter-iframe-wrap');
          var loading = document.getElementById('chapter-iframe-loading');
          var overlay = document.getElementById('chapter-iframe-overlay');
          if (iframe && wrap) {
            // Resolve URL using app.resolveChapterUrl (handles Cordova packaged paths)
            try {
              var resolved =
                typeof app !== 'undefined' && app.resolveChapterUrl
                  ? app.resolveChapterUrl(url)
                  : url;
              iframe.src = resolved || 'about:blank';
            } catch (e) {
              try {
                iframe.src = url;
              } catch (e2) {
            ...
            // wire load event to hide loading indicator
  ...
  closeChapterViewer: function () {
    var content = document.getElementById('chapter-content');
    if (content) content.innerHTML = '';
    // clear and hide iframe if present
    var iframe = document.getElementById('chapter-iframe');
    var wrap = document.getElementById('chapter-iframe-wrap');
    var loading = document.getElementById('chapter-iframe-loading');
    if (iframe) {
  ...
  retryLastChapter: function () {
    var last = app._lastChapter;
    if (!last || !last.url) return;
    var retryBtn = document.getElementById('chapter-retry');
    var content = document.getElementById('chapter-content');
    var overlay = document.getElementById('chapter-iframe-overlay');
    if (retryBtn) retryBtn.classList.add('hidden');
    if (content) content.innerHTML = '<div class="empty-note">Retrying…</div>';
    if (overlay) overlay.classList.remove('hidden');
    // re-run openChapter but try fetch only, leaving iframe as fallback handled there
    app.openChapter(last.url, last.title);
  },
  // Resolve a chapter URL to a platform-friendly path when possible.
  // (Propagated here so main/www can operate standalone.)
  resolveChapterUrl: function (url) {
    try {
      if (window.cordova && cordova.file && cordova.file.applicationDirectory) {
        ...
        return cordova.file.applicationDirectory + 'www/' + cleaned;
      }
    } catch (e) {
      /* ignore */
    }
  ...
  readRawInputs: function (formId) {
    var d = {};
    if (formId === 'me') {
      d['me-mass'] = document.getElementById('me-mass').value;
    }
    if (formId === 'ke') {
  ...
  validate: {
    ...
    ht: function () {
      ...
      if (isNaN(cp) || document.getElementById('ht-cp').value === '')
        return {
          valid: false,
          message: 'Please enter or select a specific heat capacity.',
        };
      if (cp <= 0)
        return {
          valid: false,
      ...
      if (isNaN(dt) || document.getElementById('ht-dt').value === '')
        return {
          valid: false,
          message: 'Please enter a temperature change ΔT.',
        };
      if (dt === 0)
  ...
  calculate: function (formId) {
    ...
    if (!v.valid) {
      app.showError(formId, v.message);
      return;
    }
    app.clearError(formId);
...
// Theme management: respect saved preference, fall back to system, allow toggle
var THEME_KEY = 'scicalc_theme';
...
function applyTheme(theme) {
  try {
    ...
    if (btn) {
      ...
      // accessibility state: pressed = light (on)
      try {
    ...
    console.log('Theme apply failed', e);
  }
}
...
function initTheme() {
  ...
  // No saved preference: respect prefers-color-scheme if supported
...
function toggleTheme() {
  var current = document.documentElement.classList.contains('theme-light')
    ? 'light'
    : 'dark';
...
// Expose a small hook so app initialization can wire the button
function wireThemeToggleButton() {
  var btn = document.getElementById('theme-toggle');
  if (!btn) return;
  // ensure ARIA reflects current theme
  try {
  ...
  btn.addEventListener('click', function () {
    ...
    try {
      ...
      btn.setAttribute(
        'title',
        nowLight ? 'Switch to dark theme' : 'Switch to light theme',
      );
    } catch (e) {}
  });
}
...
function onDeviceReady() {
  ...
  window.addEventListener(
    'batterystatus',
    function (info) {
      if (app.currentScreen === 'monitor' && app.currentMonitor === 'battery') {
  ...
  document.addEventListener(
    'online',
    function () {
      if (app.currentScreen === 'monitor' && app.currentMonitor === 'network') {
        app.refreshNetwork();
      }
    },
    false,
  ...
  app.showScreen('menu');
  // Initialize theme and wire the header toggle so user preference is applied
  try {
    initTheme();
  } catch (e) {
    /* ignore */
  ...
  // Wire chapter retry button if present
  try {
    var retryBtn = document.getElementById('chapter-retry');
    if (retryBtn)
      retryBtn.addEventListener('click', function () {
        app.retryLastChapter();
      });
  } catch (e) {
    /* ignore */
  }
